...
public class Enemy : MonoBehaviour
{
    ...
    public void InitEnemy()
    {
        ...
        target = GameManagerJS.Instance.player.GetComponent<Rigidbody>();
    ...
    public void MoveToTarget()
    {
        ...
        Vector3 nextVec = dirVec.normalized * status.Speed * Time.fixedDeltaTime; // 다음 벡터: fixedDeltaTime -> 프레임 영향을 받지 않게
    ...
    {
        return Vector3.Distance(transform.position, target.position) <= status.AttackRange; // 적마다 공격 사거리가 다를 수 있음
    }
    void OnDrawGizmos()
    {
